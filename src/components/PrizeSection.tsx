...
const PrizeSection = () => {
  ...
  const badges = [t("prizes.badge1"), t("prizes.badge2"), t("prizes.badge3"), t("prizes.badge4")];
  ...
  return (
    <section ref={ref} className="py-24 px-4 pb-[74px] pt-[25px]">
      <div className="max-w-5xl mx-auto">
        ...
        >
          <h2 className="font-display text-3xl sm:text-4xl md:text-[3.25rem] font-bold text-foreground mb-5 leading-tight">
            {t("prizes.headline")}
          </h2>
        ...
        {/* Prize tiers - vertical progression */}
        <div className="relative flex flex-col items-center gap-4">
          ...
          {tiers.map((tier, i) => (
            <motion.div
              ...
              className={`relative w-full ${tier.isGrand ? "max-w-xl" : "max-w-lg"}`}
            >
              <div
                className={`relative p-6 sm:p-7 rounded-2xl border transition-all duration-500 group ${
                  tier.isGrand
                    ? "bg-card border-primary/30 shadow-xl shadow-primary/10 ring-1 ring-primary/20 hover:shadow-2xl hover:shadow-primary/20 glow-primary"
                    : "bg-card border-border/50 shadow-sm hover:shadow-lg hover:border-primary/20 hover:-translate-y-0.5"
                }`}
              >
                {tier.isGrand && (
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <span
                      className={`font-display font-bold ${
                        tier.isGrand ? "text-3xl sm:text-4xl text-primary" : "text-2xl text-foreground"
                      }`}
                    >
                      {tier.monthly}
                    </span>
                    <span className="text-muted-foreground ml-2 text-sm">{tier.duration}</span>
                  </div>
                  <div className="text-right">
                    ...
                    <div
                      className={`font-display font-bold ${
                        tier.isGrand ? "text-xl text-primary" : "text-lg text-foreground"
                      }`}
                    >
                      {tier.total}
                    </div>
