  ...
  return (
    <section ref={ref} className="py-24 px-4 pb-[74px] pt-[25px]">
      <div className="max-w-4xl mx-auto">
        ...
        >
          <h2 className="font-display text-3xl sm:text-4xl md:text-[3.25rem] font-bold text-foreground leading-tight">
        ...
        <motion.div
          ...
          className="rounded-2xl border border-border/50 overflow-hidden bg-card shadow-lg shadow-foreground/[0.03]"
        >
          {/* Headers */}
          <div className="grid grid-cols-2 border-b border-border/50">
            <div className="p-5 sm:p-6 bg-primary/[0.06]">
              <span className="font-display font-bold text-primary text-sm uppercase tracking-wider">
                {t("comparison.is")}
              </span>
            </div>
            <div className="p-5 sm:p-6 bg-destructive/[0.05]">
          ...
          {comparisons.map((item, i) => (
            <motion.div
              ...
              className={`grid grid-cols-2 ${i < comparisons.length - 1 ? "border-b border-border/30" : ""} hover:bg-muted/30 transition-colors duration-200`}
            >
              <div className="flex items-center gap-3 p-4 sm:p-5">
                <div className="flex-shrink-0 w-7 h-7 rounded-full bg-primary/10 flex items-center justify-center">
                  <Check className="w-4 h-4 text-primary" />
                </div>
                <span className="text-sm sm:text-base text-foreground font-medium">{item.is}</span>
              </div>
              <div className="flex items-center gap-3 p-4 sm:p-5">
                <div className="flex-shrink-0 w-7 h-7 rounded-full bg-destructive/10 flex items-center justify-center">
                  <X className="w-4 h-4 text-destructive" />
                </div>
                <span className="text-sm sm:text-base text-muted-foreground">{item.isNot}</span>
              </div>
            </motion.div>
          ))}
        </motion.div>
